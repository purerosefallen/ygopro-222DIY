...
bool Game::Initialize() {
	...
	//modded
	device->setWindowCaption(L"YGOPro 2Pick");
	...
	wchar_t strbuf[256];
	//modded
	myswprintf(strbuf, L"YGOPro 2Pick Version:%X.0%X.%X", PRO_VERSION >> 12, (PRO_VERSION >> 4) & 0xff, PRO_VERSION & 0xf);
...
void Game::MainLoop() {
	...
	while(device->run()) {
		...
		if(cur_time >= 1000) {
			//modded
			myswprintf(cap, L"YGOPro 2Pick FPS: %d", fps);
