...
int32 scriptlib::effect_set_absolute_range(lua_State *L) {
	...
	uint32 s = lua_tonumberint(L, 3);
	uint32 o = lua_tonumberint(L, 4);
	if(playerid == 0) {
		peffect->s_range = s;
		peffect->o_range = o;
	} else {
...
int32 scriptlib::effect_set_property(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 v1 = lua_tonumberint(L, 2);
	uint32 v2 = lua_tonumberint(L, 3);
...
int32 scriptlib::effect_set_hint_timing(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 vs = lua_tonumberint(L, 2);
	uint32 vo = vs;
	if(lua_gettop(L) >= 3)
		vo = lua_tonumberint(L, 3);
...
int32 scriptlib::effect_is_has_property(lua_State *L) {
	...
	check_param(L, PARAM_TYPE_EFFECT, 1);
	effect* peffect = *(effect**) lua_touserdata(L, 1);
	uint32 tflag1 = lua_tonumberint(L, 2);
	uint32 tflag2 = lua_tonumberint(L, 3);
