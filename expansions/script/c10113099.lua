...
function c10113099.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c10113099.filter,1,nil,1-tp) and Duel.IsPlayerCanSpecialSummonMonster(tp,10113099,0,0x11,0,3000,6,RACE_FIEND,ATTRIBUTE_DARK) and Duel.GetMZoneCount(tp)>0 end
...
function c10113099.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.Destroy(g,REASON_EFFECT)~=0 and c:IsRelateToEffect(e) and Duel.IsPlayerCanSpecialSummonMonster(tp,10113099,0,0x11,0,3000,6,RACE_FIEND,ATTRIBUTE_DARK) and  Duel.GetMZoneCount(tp)>0 then
