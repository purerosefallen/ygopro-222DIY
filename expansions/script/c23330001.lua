...
function c23330001.initial_effect(c)
	...
	e4:SetDescription(aux.Stringid(23330001,0))
	e4:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_HAND)
	e4:SetTarget(c23330001.target)
	e4:SetOperation(c23330001.operation)
	c:RegisterEffect(e4)
end
function c23330001.filter(c)
	return c:IsFaceup() and c:GetLevel()>=2 and c:IsType(TYPE_TUNER)
end
function c23330001.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c23330001.filter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingTarget(c23330001.filter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.GetMZoneCount(tp)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(23330001,1))
	Duel.SelectTarget(tp,c23330001.filter,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
end
function c23330001.operation(e,tp,eg,ep,ev,re,r,rp)
