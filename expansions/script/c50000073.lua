...
function c50000073.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local flag=false
		if zone==5 then 
			flag=(Duel.CheckLocation(tp,LOCATION_MZONE,0) or Duel.CheckLocation(tp,LOCATION_MZONE,2))
		end
		if zone==20 then 
			flag=(Duel.CheckLocation(tp,LOCATION_MZONE,2) or Duel.CheckLocation(tp,LOCATION_MZONE,4))
		end
		return zone~=0 and flag and Duel.IsExistingMatchingCard(c50000073.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,zone)
	end
...
function c50000073.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if zone==0 or not flag then return end
