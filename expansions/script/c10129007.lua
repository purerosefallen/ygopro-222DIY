...
function c10129007.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(10129007,0))
...
function c10129007.thfilter(c)
	return c:IsRace(RACE_ZOMBIE) and c:GetLevel()==1 and c:IsAbleToExtraAsCost()
end
function c10129007.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c10129007.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,c10129007.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
